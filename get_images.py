import re, os, sys, json, time, uuid, random, string, base64, requests, tls_client, threading
...
def get_images():
    ...
    def __saveImage__(image):
        ...
        file = os.path.join('images', f"{uuid.uuid4().hex[:32]}.png")

        with open(file, 'wb') as f:
            f.write(data)
        
        print(f"Image: {image[:70]}***********")
...
def main():
    ...
    for i in range(1, 21):
        thread = threading.Thread(target=get_images, name=f"Thread-{i}")
