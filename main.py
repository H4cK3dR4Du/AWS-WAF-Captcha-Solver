from libs import *

class AWS_WAF_SOLVER:
    def __init__(self, siteurl: str, challenge_url: str) -> None:
        self.siteurl = siteurl
        self.challenge_url = challenge_url

        self.a = self.challenge_url.split('//')[1].split('.')[0]
        self.b = self.challenge_url.split('.')[1]
        self.c = self.challenge_url.split('/')[4]
        self.d = self.challenge_url.split('/')[5]

        self.site = f"https://{self.a}.{self.b}.us-east-1.token.awswaf.com/{self.a}/{self.c}/{self.d}"

        self.img_numbers = []
        self.predicted_imgs = []
        self.answers = []
        self.data = []
        self.img_number = 1
        self.user_agent = f'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36 Edg/120.0.0.0'

        self.client = tls_client.Session(
            client_identifier="chrome_120",
            random_tls_extension_order=True
        )

        # with open('data/proxies.txt', 'r') as f:
        #     proxies_list = f.readlines()

        # self.proxy = random.choice(proxies_list).strip()

        # self.client.proxies = {
        #     'http': f'http://{self.proxy}',
        #     'https': f'http://{self.proxy}'
        # }
        
        self.client.headers = {
            'Accept': '*/*',
            'Accept-Encoding': 'gzip, deflate, br, zstd',
            'Accept-Language': 'es',
            'Content-Type': 'text/plain;charset=UTF-8',
            'Origin': 'https://efw47fpad9.execute-api.us-east-1.amazonaws.com',
            'Priority': 'u=1, i',
            'Referer': 'https://efw47fpad9.execute-api.us-east-1.amazonaws.com/',
            'Sec-Ch-Ua': '"Not/A)Brand";v="8", "Chromium";v="120", "Microsoft Edge";v="120"',
            'Sec-Ch-Ua-Mobile': '?0',
            'Sec-Ch-Ua-Platform': '"Windows"',
            'Sec-Fetch-Dest': 'empty',
            'Sec-Fetch-Mode': 'cors',
            'Sec-Fetch-Site': 'cross-site',
            'User-Agent': self.user_agent
        }

    def __makeNumber__(self, number) -> float:
        floatten = random.random() * 2 + number
        return round(floatten, 16)

    def __values__(self) -> None:
        r = self.client.get(self.siteurl)

        pattern = r'window\.gokuProps\s*=\s*\{\s*"key"\s*:\s*"([^"]+)"\s*,\s*"iv"\s*:\s*"([^"]+)"\s*,\s*"context"\s*:\s*"([^"]+)"\s*\};'
        match = re.search(pattern, r.text)

        self.key = match.group(1)
        self.iv = match.group(2)
        self.context = match.group(3)

        # print("Got key ->", self.key)
        # print("Got Iv ->", self.iv)
        # print("Got Context ->", self.context)

    def __challengeValues__(self) -> None:
        r = self.client.get(f"{self.site}/inputs?client=browser")

        self.input = r.json()['challenge']['input']
        self.hmac = r.json()['challenge']['hmac']
        self.region = r.json()['challenge']['region']
        self.challenge_type = r.json()['challenge_type']
        self.difficulty = r.json()['difficulty']

        # print("Got input ->", self.input)
        # print("Got hmac ->", self.hmac)
        # print("Got region ->", self.region)
        # print("Got challenge_type ->", self.challenge_type)
        # print("Got difficulty ->", self.difficulty)

    def __getChallenge__(self) -> None:
        payload = {
            "challenge": {
                "hmac": self.hmac,
                "input": self.input,
                "region": self.region,
            },
            "checksum": "EC2D2BD3",
            "client": "Browser",
            "domain": "efw47fpad9.execute-api.us-east-1.amazonaws.com",
            "existing_token": None,
            "goku_props": {
                "context": self.context,
                "iv": self.iv,
                "key": self.key,
            },
            "metrics": [
                {"name": "2", "value": self.__makeNumber__(1.7), "unit": "2"},
                {"name": "100", "value": 1, "unit": "2"},
                {"name": "101", "value": 2, "unit": "2"},
                {"name": "102", "value": 1, "unit": "2"},
                {"name": "103", "value": 40, "unit": "2"},
                {"name": "104", "value": 2, "unit": "2"},
                {"name": "105", "value": 0, "unit": "2"},
                {"name": "106", "value": 0, "unit": "2"},
                {"name": "107", "value": 1, "unit": "2"},
                {"name": "108", "value": 0, "unit": "2"},
                {"name": "undefined", "value": 1, "unit": "2"},
                {"name": "110", "value": 1, "unit": "2"},
                {"name": "111", "value": 21, "unit": "2"},
                {"name": "112", "value": 1, "unit": "2"},
                {"name": "undefined", "value": 0, "unit": "2"},
                {"name": "3", "value": self.__makeNumber__(16), "unit": "2"},
                {"name": "7", "value": 0, "unit": "4"},
                {"name": "1", "value": self.__makeNumber__(96), "unit": "2"},
                {"name": "4", "value": 101.5, "unit": "2"},
                {"name": "5", "value": self.__makeNumber__(0.1), "unit": "2"},
                {"name": "6", "value": 198, "unit": "2"},
                {"name": "0", "value": self.__makeNumber__(518), "unit": "2"},
                {"name": "8", "value": 1, "unit": "4"}
            ],
            "signals": [
                {"name": "KramerAndRio", "value": {
                    "Present": "iT7BYGRpkjOQzq7O::3454d6468bc7cc819a749175d5bca7e0::",
                }},
            ],
            "solution": str(random.randint(700, 868))
        }

        r = self.client.post(f"{self.site}/verify", json=payload)
        if r.json()['inputs'] != "null":
            self.token = r.json()['token']
            # print("Got token ->", self.token)
        else:
            return
    
    def __sessionStorage__(self) -> None:
        payload = {
            "awswaf_session_storage": None,
            "checksum": "7E116771",
            "client": "Browser",
            "existing_token": self.token,
            "metrics": [
                {"name": "12", "value": self.__makeNumber__(0.2), "unit": "2"},
                {"name": "200", "value": 1, "unit": "2"},
                {"name": "201", "value": 2, "unit": "2"},
                {"name": "13", "value": self.__makeNumber__(5.8), "unit": "2"},
                {"name": "10", "value": 0, "unit": "4"},
                {"name": "9", "value": 0, "unit": "4"},
                {"name": "11", "value": self.__makeNumber__(19.7), "unit": "2"},
            ],
            "signals": [
                {"name": "KramerAndRio", "value": {
                    "Present": "zWctqBru/CzYe+H4::772711810aee175f7af5bdf529971cb8::",
                }},
            ]
        }

        r = self.client.post(f"{self.site}/telemetry", json=payload)
        if "token" in r.json():
            self.token = r.json()['token']
            self.interval = r.json()['next_interval']
            self.awswaf_session_storage = r.json()['awswaf_session_storage']
            
            # print("Got token ->", self.token)
            # print("Got interval ->", self.interval)
            # print("Got awswaf_session_storage ->", self.awswaf_session_storage)
        else:
            return
        
    def __getImages__(self) -> None:
        r = self.client.get(f"https://41bcdd4fb3cb.610cd090.us-east-1.captcha.awswaf.com/41bcdd4fb3cb/0d21de737ccb/cd77baa6c832/problem?kind=visual&domain=efw47fpad9.execute-api.us-east-1.amazonaws.com&locale=es")
        
        if "validity_duration" in r.json() and r.json()["validity_duration"] == 60:
            self.problem_type = r.json()['problem_type']
            if self.problem_type == "toycarcity":
                print("toy carcity not supported yet nigga")
                return
            else:
                self.solve_iv = r.json()['state']['iv']
                self.solve_payload = r.json()['state']['payload']
                self.solve_key = r.json()['key']
                self.hmac_tag = r.json()['hmac_tag']
                self.images = json.loads(r.json()['assets']['images'])
                self.target = r.json()['assets']['target'].replace('["', '').replace('"]', '').capitalize()
                self.target0 = r.json()['localized_assets']['target0'].capitalize()
                self.validity = r.json()['validity_duration']

            Logger.captcha(f"CAP,Please click on the images that contain -> {self.target},9 Images")
            for nigger, image in enumerate(self.images):
                self.__solveImages__(image)
        else:
            return
        
    def __solveImages__(self, image) -> None:
        data = base64.b64decode(image)
        file = os.path.join('temp', f"{uuid.uuid4().hex[:32]}.png")

        with open(file, 'wb') as f:
            f.write(data)
        
        answers = []
        images = []

        model = tf.keras.models.load_model('model.keras')
        test_datagen = tf.keras.preprocessing.image.ImageDataGenerator(rescale=1./255)

        test_generator = test_datagen.flow_from_directory(
            'dataset',
            target_size=(100, 100),
            batch_size=1,
            class_mode='categorical',
            shuffle=False
        )

        img = load_img(file, target_size=(100, 100))
        img_array = img_to_array(img)
        img_array = np.expand_dims(img_array, axis=0)
        img_array = img_array / 255.0

        predictions = model.predict(img_array)
        predicted_class_index = np.argmax(predictions[0])

        class_indices = test_generator.class_indices
        predicted_class = [k for k, v in class_indices.items() if v == predicted_class_index][0]

        images.append(f"{file} -> {predicted_class}")
        if predicted_class == self.target:
            answers.append(file)
            self.data.append(True)
            self.img_numbers.append(self.img_number)
        else:
            self.data.append(False)

        # for image in images:
            # print(f"Predicted Image -> {image}")
        
        os.remove(file)
        self.img_number += 1

    def __solveChallenge__(self) -> None:
        # print(self.data)
        # print(self.img_numbers)

        imgg = ""
        total = 0

        for img in self.img_numbers:
            total += 1
            imgg += f"{img},"

        if imgg.endswith(","):
            imgg = imgg[:-1]

        split_values = imgg.split(",")

        if total == 2:
            cl_solution = [
                int(split_values[0]),
                int(split_values[1]),
            ]
        elif total == 3:
            cl_solution = [
                int(split_values[0]),
                int(split_values[1]),
                int(split_values[2])
            ]
        elif total == 4:
            cl_solution = [
                int(split_values[0]),
                int(split_values[1]),
                int(split_values[2]),
                int(split_values[3])
            ]
        elif total == 5:
            cl_solution = [
                int(split_values[0]),
                int(split_values[1]),
                int(split_values[2]),
                int(split_values[3]),
                int(split_values[4])
            ]

        cl_solution_2 = []
        for solution in cl_solution:
            solution -= 1
            cl_solution_2.append(solution)

        Logger.pretty(f"ANS,Solved Images,Keras")

        payload = {
            "client_solution": cl_solution_2,
            "goku_props": {
                "context": self.context,
                "iv": self.iv,
                "key": self.key,
            },
            "hmac_tag": self.hmac_tag,
            "key": self.solve_key,
            "locale": "es",
            "metrics": {
                "solve_time_millis": random.randint(4899, 7047),
            },
            "state": {
                "iv": self.solve_iv,
                "payload": self.solve_payload,
            }
        }

        r = self.client.post(f"https://41bcdd4fb3cb.610cd090.us-east-1.captcha.awswaf.com/41bcdd4fb3cb/0d21de737ccb/cd77baa6c832/verify", json=json.dumps(payload))
        if r.json()['num_solutions_provided'] == 1:
            self.captcha = r.json()['captcha_voucher']
            Logger.success(f"SUC,Solved Captcha,{self.captcha[:60]}********")
        else:
            Logger.failed(f"ERR,Failed Solving Captcha,Invalid Answers")

    def __validateCaptcha__(self) -> None:
        payload = {
            "captcha_voucher": self.captcha,
            "existing_token": self.token
        }

        r = self.client.post("https://41bcdd4fb3cb.610cd090.us-east-1.token.awswaf.com/41bcdd4fb3cb/0d21de737ccb/cd77baa6c832/voucher", json=payload)
        if r.status_code != 200:
            Logger.failed(f"ERR,Failed Validating Captcha,{self.captcha[:60]}********")
        else:
            Logger.warning(f"DBG,Validated Captcha,{self.captcha[:60]}********")

def solve():
    try:
        aws = AWS_WAF_SOLVER(
            siteurl="https://efw47fpad9.execute-api.us-east-1.amazonaws.com/latest",
            challenge_url="https://41bcdd4fb3cb.610cd090.us-east-1.token.awswaf.com/41bcdd4fb3cb/0d21de737ccb/cd77baa6c832/challenge.js"
        )

        aws.__values__()
        aws.__challengeValues__()
        aws.__getChallenge__()
        aws.__sessionStorage__()
        aws.__getImages__()
        aws.__solveChallenge__()
        aws.__validateCaptcha__()
    except Exception as e:
        print(e)
        solve()

if __name__ == "__main__":
    solve()